...
def generate_launch_description():
    return LaunchDescription([
        ...
        # Nodo del driver RPLIDAR
        Node(
            package='rplidar_ros',
            executable='rplidar_composition',
            name='rplidar_node',
            parameters=[{
                'serial_port': '/dev/ttyUSB0',  # Cambia si tu puerto es diferente
                'serial_baudrate': 115200,
                'frame_id': 'laser',
                'inverted': False,
                'angle_compensate': True,
                'scan_mode': 'Standard'  # Usa un modo compatible, no 'Express'
            }],
            output='screen'
        ),
        ...
        # Lanzar RViz2 con visualización opcional
        ExecuteProcess(
            cmd=['rviz2', '-d', os.path.join(
                os.path.expanduser('~'),
                'ros2_ws/src/lidar_pose_estimation/rviz/lidar_pose_view.rviz')],
            output='screen'
        )
    ])
